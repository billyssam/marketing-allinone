...
export interface TitleStyle {
   ...
   *
   * 업종(offering)별로 나눠 둔다. 예시 하나를 전 업종에 쓰면 카페 예시가 미용실·헬스장
   * 글까지 끌어당긴다("구조만 참고하라"고 써도 few-shot 내용은 강하게 작용한다).
   * 자영업 업종 43종 중 음식은 일부일 뿐이라 나머지가 계속 남의 옷을 입게 된다.
   */
  example: Record<OfferingKind, string>;
...
const TITLE_STYLES: TitleStyle[] = [
  {
    ...
    example: {
      menu: '무더위에 지칠 땐 어디로 가야 할까요? 옥천 쿵더쿵의 여름 한 잔',
      product: '올여름 뭘 입어야 할지 고민이신가요? 성수 라운드의 리넨 셔츠',
      service: '아침마다 머리가 말썽인가요? 강남 스타일링룸의 새 커트',
      booking: '운동을 몇 번이나 포기하셨나요? 판교 코어짐의 4주 프로그램',
    },
    check: (t) => t.includes('?'),
  },
  {
    ...
    example: {
      menu: '버터 향이 번지는 오후, 옥천 쿵더쿵에서 만난 크로플',
      product: '햇살에 비친 리넨의 결, 성수 라운드에서 만난 여름 셔츠',
      service: '가위 소리가 잦아든 순간, 강남 스타일링룸의 거울 앞',
      booking: '숨이 차오르는 마지막 3분, 판교 코어짐의 새벽',
    },
  },
  {
    key: 'number',
    ...
    example: {
      menu: '단 5분이면 충분한 휴식, 옥천 쿵더쿵의 세 가지 여름 메뉴',
      product: '한 벌로 3주를 나는 법, 성수 라운드의 여름 셔츠',
      service: '40분이면 달라지는 인상, 강남 스타일링룸의 커트 세 가지',
      booking: '주 2회 8주, 판교 코어짐이 제안하는 여름 루틴',
    },
  ...
  },
  {
    ...
    example: {
      menu: '퇴근길이 유난히 길게 느껴진 날, 옥천 쿵더쿵에 들렀습니다',
      product: '옷장을 열어도 입을 게 없던 아침, 성수 라운드에 들렀습니다',
      service: '거울 볼 때마다 한숨이 나오던 날, 강남 스타일링룸에 갔습니다',
      booking: '계단만 올라도 숨이 차던 무렵, 판교 코어짐을 시작했습니다',
    },
  },
  {
    key: 'plain',
    ...
    example: {
      menu: '쿵더쿵 카페의 여름 신메뉴, 눈꽃빙수가 나왔습니다',
      product: '성수 라운드의 여름 신상, 리넨 셔츠가 들어왔습니다',
      service: '강남 스타일링룸의 여름 커트, 이렇게 달라집니다',
      booking: '판교 코어짐의 여름 프로그램, 이번 주 시작합니다',
    },
  },
  {
    key: 'topic',
    rule: '오늘의 중심 소재(메뉴·상품·시술 등)를 제목 맨 앞에 세울 것. 상호는 뒤에 가볍게.',
    example: {
      menu: '수제대추차 한 잔에 담긴 정성, 옥천 쿵더쿵',
      product: '리넨 셔츠 한 장에 담긴 계절, 성수 라운드',
      service: '레이어드 컷에 담긴 계산, 강남 스타일링룸',
      booking: '코어 운동 8주에 담긴 변화, 판교 코어짐',
    },
  },
];
...
export function titleDirective(
  ...
  return (
    ...
    `  · 형식 예시(내용은 따라 쓰지 말고 구조만 참고): "${style.example[offering]}"\n` +
    (style.key === 'plain'
