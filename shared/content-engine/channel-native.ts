...
const CHANNEL_BRIEF: Record<string, string> = {
  ...
  naver_place:
    '네이버 플레이스 소식. 정보 중심·간결. 지금 방문할 이유(신메뉴·시즌·영업정보) 강조. 이모지 최소. **150~250자**.',
  danggeun:
  ...
  kakao_channel:
    '카카오톡 채널 메시지. 친구 추가한 고객의 알림창에 뜨는 글. 첫 문장에서 용건이 바로 드러나야 하고(뭐가 새로운지·언제까지인지), 광고 느낌보다 단골에게 귀띔하듯. 문의·방문으로 잇는 짧은 한 줄로 마무리. 이모지 0~2개. **150~300자**.',
  facebook:
    '페이스북 페이지 게시물. 인스타보다 문장을 길게 써도 되고 설명이 친절해야 한다(중장년 이용자 비중이 큼). 스토리 한 토막에 **원본에 적힌** 실용 정보(가격·영업시간·위치)를 자연스럽게 섞고, 마지막은 방문·문의로 잇는다. 원본에 없는 시간·가격은 절대 적지 말 것. 해시태그는 쓰더라도 2~3개까지. **400~600자**.',
  google_business:
