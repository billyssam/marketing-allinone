...
/**
 * 지역명 목록 — 해시태그에 지역을 지어냈는지 판정할 때만 쓴다.
 *
 * 광역 지자체만 두었더니 `#강남수학` `#역삼수학`을 못 잡았다(2026-08-18 실측, 주소 없는 학원).
 * 모델은 업종별로 **정해진 동네를 기본값처럼** 뱉는다 — 학원이면 강남·목동, 카페면 성수·연남.
 * 전국 행정구역을 다 넣을 수는 없으니, 광역 + 서울 자치구 + 모델이 자주 부르는 상권을 담는다.
 * 완전하지 않다 — 그래서 프롬프트에도 "지역을 지어내지 말라"를 넣어 1차로 막고 여기서 걸러낸다.
 */
const REGIONS = [
  // 광역
  ...
  '영등포', '용산', '은평', '종로', '중랑',
  // 모델이 기본값처럼 부르는 상권·신도시
  '역삼', '삼성동', '선릉', '논현', '청담', '압구정', '성수', '연남', '홍대', '이태원',
  '목동', '중계', '대치', '잠실', '판교', '분당', '일산', '수지', '동탄', '광교', '해운대', '서면',
];
 ...
 * (2026-08-18): 주소 없는 학원 블로그에 `#강남수학` `#역삼수학`.
 * 손님이 엉뚱한 동네에서 찾게 되는 오류인데, 숫자 날조 경보는 숫자만 보므로 지명은 그냥 통과한다.
 *
