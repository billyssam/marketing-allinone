...
| 08:30 | `owner-watch` | **신규 가입·이탈 감지** — 새 사장님은 그날 안에, 7일 넘게 안 온 분은 이탈 신호로. 상세는 운영자 텔레그램(공개 저장소라 이슈엔 인원수만) |
| 15:00 + 코드 push마다 | `screen-checks` | **가짜 대량 데이터로 화면 검증** — 리뷰 120·단골 529·글 3개월치를 넣고 실제 브라우저로 18개 항목 |
