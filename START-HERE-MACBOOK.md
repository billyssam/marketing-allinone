...
## 🗓 내일(다음 세션) 먼저 확인할 것 — 게이트 열림

1. **⏰ 오늘 밤~내일 07:30 크론** = 이 세션 콘텐츠 작업의 실 Gemini E2E 검증.
   demo(쿵더쿵)에 facebook·threads 연결해둠 → **4채널(블로그+인스타+페북+스레드)**
   생성 + **각도×시점×소재×길이 변주**가 실제로 적용됐는지 `/posts`·DB로 확인.
   (오늘 Gemini 일일한도 소진으로 로직만 유닛검증, 실 생성은 크론이 완결)
2. **당근·밴드·카카오채널** = `exmbpietyadkjnunrhka`에서 `0005` SQL 돌리면 코드 4곳
   추가로 즉시 열림(사용자 계정 필요).
3. **컴포저 각도 칩 시각검증** = 브라우저팬 하이드레이션 글리치로 이번 세션 보류.
   비글리치 환경서 대시보드 "오늘 글 생성" → 각도 칩 눈으로 확인.

## 🦴 콘텐츠 품질 대폭 강화 (2026-07-21 오후 세션, ~15커밋)

**골격 5개 + 콘텐츠 4중 변주** (전부 커밋·배포, 테스트 55+ pass):
- 적응 엔진(업종→채널·톤) · offerings 모델(모든 업종 실소재) · 적응 대시보드 ·
  연결 채널 전부 생성(facebook/threads/google) · near-100 전 화면.
- **콘텐츠 반복 방지 = 각도(방향)×시점(계절·이벤트)×중심소재×길이 4중 로테이션**
  (`shared/content-engine/angles.ts`·`seasonal.ts`, dailyDirective 통일).
  데일리 크론·수동 생성·컴포저 칩 전부 반영.
- 재방문 메시지도 시점(크리스마스 등) 반영.

## 🦴 골격 작업 중 (2026-07-21, HEAD `67c608c`) — 여기서 이어감

**골격2 완료 = "이 사업이 파는 것" 범용 모델 (offerings)** (커밋까지 배포됨):
- `shared/content-engine/offerings.ts` = 메뉴/상품/시술/프로그램 통합(StoreOffering).
  `resolveOfferings()` 사장님 관리분 우선·없으면 크롤메뉴 폴백. 콘텐츠 엔진이 이걸 주입.
- 설정 페이지에 **판매 항목 편집기**(업종별 라벨: 메뉴/상품/시술…, 이름·가격 행 추가).
  카페는 크롤 메뉴로 프리필, 비-카페는 여기 입력이 유일한 실제 소재.
- 🔴 updateStore 업종 리셋 버그 수정(6개 화이트리스트→택소노미 검증).
- 저장 = `stores.brand_tone.offerings` jsonb (DDL 접근 없어 신규 테이블 대신).
- **E2E 검증**: 미용실(크롤 없음)+서비스3종 입력 → 생성 글이 남성컷·염색·매직+가격
  전부 + 서비스 톤. 카페는 크롤메뉴+수동추가 둘 다 반영.

**다음 골격 후보** (마감 아님, 구조):
1. **대시보드가 업종별로** — 지금 리뷰 중심. 판매형=주문/매출, 예약형=예약 지표로 적응.
2. **콘텐츠 소재 다양화** — offering별 사진·소재를 더 깊이(제품샷/비포애프터).
3. **전용 offerings 테이블** — jsonb→테이블 마이그레이션(상품 많은 업종 대비, 사용자 SQL 필요).
> 방향은 사용자 확인 후. (골격은 확인하고 짓기 — 여태 방향 여러 번 정정받음.)

---

## 🦴 골격1 (2026-07-20, `147c2db`) — 적응 엔진
